    ...
    {:else}
      <div class="video-grid">
        <div class="video-box local">
          ...
          {#if connectionState === 'CONNECTED'}
            {#if fullscreenTarget === 'local'}
              {#if showExitButton}
                <button class="exit-fullscreen-btn icon-btn" title="Exit Fullscreen" onclick={() => toggleFullscreen(null)}>
            ...
            {:else if !isFullscreen}
              <button class="fullscreen-btn icon-btn" title="Fullscreen" onclick={() => toggleFullscreen('local')}>
        ...
        <div class="video-box remote">
          ...
          {#if connectionState === 'CONNECTED'}
            {#if fullscreenTarget === 'remote'}
              ...
              {/if}
            {:else if !isFullscreen}
              <button class="fullscreen-btn icon-btn" title="Fullscreen" onclick={() => toggleFullscreen('remote')}>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 3H5a2 2 0 0 0-2 2v3"/><path d="M21 8V5a2 2 0 0 0-2-2h-3"/><path d="M3 16v3a2 2 0 0 0 2 2h3"/><path d="M16 21h3a2 2 0 0 0 2-2v-3"/></svg>
              </button>
            {/if}
          {/if}
